(function () {
  ...
  g.prototype = {
    ...
    onKeyDown: function (z) {
      ...
      if (
        !this.crashed &&
        (g.keycodes.JUMP[z.keyCode] ||
          z.type == g.events.TOUCHSTART ||
          z.type == g.events.GAMEPADCONNECTED)
      ) {
        if (!this.activated) {
          this.loadSounds();
          this.activated = true;
      ...
      if (
        this.crashed &&
        z.type == g.events.TOUCHSTART &&
        z.currentTarget == this.containerEl
      ) {
        this.restart();
      }
    ...
    onKeyUp: function (C) {
      ...
      var A =
        g.keycodes.JUMP[B] ||
        C.type == g.events.TOUCHEND ||
        C.type == g.events.MOUSEDOWN;
      if (this.isRunning() && A) {
        this.tRex.endJump();
      } else {
        if (g.keycodes.DUCK[B]) {
          ...
          if (this.crashed) {
            var z = p() - this.time;
            if (
              g.keycodes.RESTART[B] ||
              this.isLeftClickOnCanvas(C) ||
              (z >= this.config.GAMEOVER_CLEAR_TIME && g.keycodes.JUMP[B])
            ) {
              this.restart();
            }
          } else {
    ...
    isLeftClickOnCanvas: function (z) {
      return (
        z.button != null &&
        z.button < 2 &&
        z.type == g.events.MOUSEUP &&
        z.target == this.canvas
      );
    },
    raq: function () {
      if (!this.drawPending) {
  ...
  function u(H, B, J) {
    ...
    var I = new o(
      H.xPos + 1,
      H.yPos + 1,
      H.typeConfig.width * H.size - 2,
      H.typeConfig.height - 2
    );
    if (J) {
      j(J, G, I);
    }
  ...
  i.prototype = {
    init: function () {
      ...
      this.groundYPos =
        g.defaultDimensions.HEIGHT - this.config.HEIGHT - g.config.BOTTOM_PAD;
    ...
    update: function (z, A) {
      ...
      if (this.playingIntro && this.xPos < this.config.START_X_POS) {
        this.xPos += Math.round(
          (this.config.START_X_POS / this.config.INTRO_DURATION) * z
        );
      }
    ...
    draw: function (z, E) {
      ...
      var C = E;
      var A =
        this.ducking && this.status != i.status.CRASHED
    ...
    updateJump: function (A, C) {
      ...
      if (this.speedDrop) {
        this.yPos += Math.round(
          this.jumpVelocity * this.config.SPEED_DROP_COEFFICIENT * z
        );
      } else {
  ...
  v.prototype = {
    ...
    updateClouds: function (A, C) {
      ...
      if (E) {
        ...
        if (
          ...
          this.dimensions.WIDTH - D.xPos > D.cloudGap &&
    ...
    updateObstacles: function (z, C) {
      ...
      if (this.obstacles.length > 0) {
        ...
        if (
          D &&
          !D.followingObstacleCreated &&
          D.isVisible() &&
          D.xPos + D.width + D.gap < this.dimensions.WIDTH
        ) {
    ...
    addNewObstacle: function (B) {
      ...
      if (this.duplicateObstacleCheck(z.type) || B < z.minSpeed) {
        ...
        this.obstacles.push(
          new q(this.canvasCtx, z, A, this.dimensions, this.gapCoefficient, B)
        );
    ...
    addCloud: function () {
      this.clouds.push(
        new m(this.canvas, this.spritePos.CLOUD, this.dimensions.WIDTH)
      );
    },
  };
})();
